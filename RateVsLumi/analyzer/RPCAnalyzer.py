...
fout            = ROOT.TFile.Open(outFolder + "/" + "fileout" + part + ".root", "RECREATE")


print(f"Part:           {part}")
print(f"Input files:    {str(file_list)}")
print("Output file:     " + outFolder + "/" + "fileout" + part + ".root")
...
df      = uproot.concatenate([infile + ":demo/hit_tree" for infile in file_list], library="pd") #, cut = que_) # cut = que_[:-2]
...
def hit_hist(name, que ="", coll_region=0):
        if coll_region!=0:
                df_ = df.query(que+" & coll_region=="+str(coll_region))
        else:
                df_ = df.query(que)
        nbin = 100
